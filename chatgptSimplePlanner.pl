...
clockwise(RLoc, NewRLoc).

% Move counterclockwise
action(mcc, state(RLoc, RHC, SWC, MW, RHM), state(NewRLoc, RHC, SWC, MW, RHM)) :-
counterclockwise(RLoc, NewRLoc).

% Pickup coffee
action(puc, state(RLoc, no, SWC, MW, RHM), state(RLoc, yes, SWC, MW, RHM)).

% Deliver coffee
action(dc, state(RLoc, yes, yes, MW, RHM), state(RLoc, no, no, MW, RHM)).

% Pickup mail
action(pum, state(RLoc, RHC, SWC, yes, no), state(RLoc, RHC, SWC, no, yes)).

% Deliver mail
action(dm, state(RLoc, RHC, yes, MW, yes), state(RLoc, RHC, no, no, no)).
% Plan function
plan(State, State, []). % To achieve State from State itself, do nothing
